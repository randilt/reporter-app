...
const statusConfig: Record<SyncStatus, { icon: typeof CheckCircle; color: string; label: string }> = {
  ...
  synced: { icon: CheckCircle, color: 'text-synced', label: 'Synced' },
  failed: { icon: AlertCircle, color: 'text-failed', label: 'Failed' }
};
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mb-2">
            <span className={cn("px-2 py-0.5 rounded text-xs font-bold", severity.color)}>
              {severity.label}
            </span>
        ...
        <div className="flex flex-col items-end gap-2">
          <div className={cn("flex items-center gap-1.5", status.color)}>
            <StatusIcon className={cn("w-4 h-4", report.syncStatus === 'pending' && "status-pulse")} />
...
export const ReportsList = ({ filter = 'all' }: ReportsListProps) => {
  ...
  const filteredReports = filter === 'all' 
    ? reports 
    : reports.filter(r => r.syncStatus === filter);

  if (filteredReports.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        <div className="w-16 h-16 rounded-full bg-muted flex items-center justify-center mb-4">
          {filter === 'pending' && <Clock className="w-8 h-8 text-muted-foreground" />}
          {filter === 'synced' && <CheckCircle className="w-8 h-8 text-muted-foreground" />}
          {filter === 'failed' && <AlertCircle className="w-8 h-8 text-muted-foreground" />}
          {filter === 'all' && <ChevronRight className="w-8 h-8 text-muted-foreground" />}
        </div>
        <p className="text-muted-foreground">
          {filter === 'all' 
            ? 'No reports yet. Create your first incident report.'
            : `No ${filter} reports.`
          }
        </p>
      </div>
